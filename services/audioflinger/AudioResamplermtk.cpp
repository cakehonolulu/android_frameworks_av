...
{
    ALOGD("init outsampleRate %u, intsampleRate %u",mSampleRate, SrcSampleRate);
...
{
    //ALOGD("setSampleRate %u",inSampleRate); log too much when playback
    if(mBliSrc != NULL)
    {
...
void AudioResamplerMtk::resample(int32_t* out, size_t outFrameCount,
    ...
    if(mOutputBuf == NULL) // size of this buffer can only be determined at this time.
    {
        ...
        if(ret != OK)
        {
            ALOGE("resample fail to configBuf, There is no memory");
            return ;
        }
    }
    ...
    ALOGV("starting resample outFrameCount %d, outputSampleCount %d, inFrameCount %d",
    ...
    while(outFrameCount){
        if(mBuffer.frameCount == 0) 
        {
            ...
            if (mBuffer.raw == NULL) {
                ALOGD("AudioResamplerMtk::resample() can't get buffer");
                reset();
        ...
        if(mBliSrc)
        {
            ALOGV("+resample inBufSize %d, outBufSize %d",inBufSize,outBuffSize);
            consume = mBliSrc->resample(in,&inBufSize, mOutputBuf + outputIndex,&outBuffSize);
            ALOGV("-resample consume %d, inBufSize %d, outBufSize %d",consume,inBufSize,outBuffSize);
        }
        else
        {
            ALOGE("Can not Resampler, pad 0");
            consume = inBufSize;
